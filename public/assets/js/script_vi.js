...
$("#error").empty();
      ...
      var brand=$("#brand").val();
      var year1=$("#year1").val();
      var image_upload=$("#image2").val();
     
   
 if($.trim(email).length>0 &&  $.trim(contact).length>0 && $.trim(title).length>0 &&  $.trim(price).length>0 && $.trim(cat_id).length>0 && $.trim(sub_cat_id).length>0 && 
    $.trim(streets).length>0 && $.trim(description).length>0) {
      ...
      if ($.trim(cat_id)==real_estate_id) {
     	...
     	$.ajax({
		url: "add", // Url to which the request is send
	...
	*/
     if(!$("#image2").val()){
      $('#upload_image2').css("border", "1px solid red");
     }else{ $('#upload_image2').css("border", "1px solid #ccc")}
